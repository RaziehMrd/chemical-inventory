...
UPLOAD_DIR = os.environ.get("UPLOAD_DIR", "/opt/render/project/src/data/uploads")
...
def upsert_chemical(name: str, amount: float, unit: str, location: str, notes: str = ""):
    name, unit, location, notes = name.strip(), unit.strip(), location.strip(), notes.strip()
    with Session(engine) as sess:
...
def list_requests(status: str | None = None):
    with Session(engine) as sess:
        if status:
            stmt = text("""
                SELECT r.id, c.name, r.quantity, r.first_name, r.surname, r.requester_email, r.status, r.created_at
                FROM requests r
...
def save_uploaded_file(upload, prefix: str = "attach") -> str:
    """Save an uploaded file to UPLOAD_DIR with a unique-ish name, return path or ''."""
    if upload is None:
        return ""
    stem = re.sub(r"[^A-Za-z0-9_.-]", "_", upload.name)
    filename = f"{prefix}_{int(datetime.utcnow().timestamp())}_{stem}"
...
def guess_unit(material: str, specs: str) -> str:
    """Heuristic unit suggestion based on keywords (powders vs liquids, acids, solvents, etc.)."""
...
def cas_hint():
    st.caption(
        f"Need help finding a CAS number? See the public CAS lookup: "
        f"[CAS Common Chemistry]({CAS_HELP_URL})"
    )
...
with tabs[0]:
    ...
    if data:
        ...
        # -------- Purchase Request (appears only when no results) --------
        ...
        with req_tab_required:
            with st.form("purchase_required_form"):
                ...
                specifications = st.text_area(
                    ...
                    height=100,
                    help="Provide any technical requirements that procurement should match."
                )
                ...
                with qcol1:
                    amount = st.number_input("Amount ★", min_value=0.0, step=0.1, format="%.3f", help="Numeric value only.")
                with qcol2:
                    ...
                    try:
                        ...
                        default_idx = 0
                    unit = st.selectbox("Unit ★", unit_options, index=default_idx, help="Choose a unit.")
        ...
        with req_tab_optional:
            ...
            attachment = st.file_uploader(
                ...
                help="Attach any supporting file. It will be stored securely on the server."
            )
        ...
        if submit_purchase_required:
            ...
            # Basic CAS sanity check: digits-digits-d
            if not re.match(r"^\d{2,7}-\d{2}-\d$", cas_number.strip()):
                errs.append("Valid CAS number (e.g., 64-17-5)")
